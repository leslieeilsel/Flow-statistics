<?php
    ...
    foreach($dateTimes as $key=>$datetime){
        $period = $db->query("SELECT dept,count(dept) as deptcnt FROM [dbo].[RTPass] WHERE VisitTime BETWEEN '".date('Y-m-d')." ".$datetime[0]."' AND '".date('Y-m-d')." ".$datetime[1]."'and dept !='未知部门' GROUP BY dept");
